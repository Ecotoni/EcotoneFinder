                             ...
                             cex.x = 12)
{
  ...
  if (method == "fanny") {
    if (normalized == "species") {
      GGData <- as.data.frame(t(vegan::decostand(t(ecotonefinder$fanny$centers), method = "total")*100))
    }
    if (normalized == "cluster") {
      GGData <- as.data.frame(vegan::decostand(ecotonefinder$fanny$centers, method = "total")*100)
      GGData <- GGData[,as.numeric(which(base::colSums(GGData) > threshold))]
    }
    if (normalized == "none") {
      GGData <- as.data.frame(ecotonefinder$fanny$centers)
      GGData <- GGData[,as.numeric(which(base::colSums(GGData) > threshold))]
    }
    GGData$Cluster <- base::rownames(GGData)
    GGData <- reshape::melt(GGData, id.vars = "Cluster")
  ...
  GG <- ggplot2::ggplot(GGData) +
    ggplot2::geom_col(ggplot2::aes(x = GGData[["Species"]], y = GGData[["Contribution"]], group = GGData[["Cluster"]], fill = GGData[["Cluster"]]),
                      position = position) +
