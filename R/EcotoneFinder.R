                           ...
                           diversity = c("shannon", "richness", "expShannon", "pielou", "all"), na.rm = FALSE)
{
  ...
  if (is.numeric(dist) == FALSE && assertthat::is.date(dist) == FALSE) {
    stop("dist must be a either a numeric or a date vector or column.")
  }
  if (any(method %in% c("fanny", "vegclust", "cmeans", "all")) && is.null(groups)) {
  ...
  if (any(method == "fanny") || any(method == "all")) {
    ...
    FannyCenters <- list()
    FannyCenters <- matrix(NA, groups,
                           ncol(data))
    for (j in 1:groups){
      FannyCenters[j,] <- colSums(Fanny$membership[,j] * Fanny$data) / sum(Fanny$membership[,j])
    }
    colnames(FannyCenters) <- colnames(Fanny$data)
    list_output[[paste("fanny")]][["centers"]] <- FannyCenters
  }
