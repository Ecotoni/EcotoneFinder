                       ...
                       return.plot = TRUE)
{
  ...
  if (any(is.na(names(ecotonefinder[intersect(m, method)])))) {
    stop("ecotonefinder must comntain the elements required in 'method'.")
  }
  if (length(intersect(s, method)) != 0 && any(is.na(names(slope[intersect(s, method)])))) {
    stop("slope must comntain the elements required in 'method'.")
  }
  GG <- list()
  ...
  if (is.null(facet) == FALSE) {
    ...
    for (i in method) {
      if (is.null(ncol(GG[[i]]))) {
        GG[[i]] <- data.frame("Distance" = GG[["distance"]], "Type" = i, i = GG[[i]])
      } else {
  ...
  if (is.null(facet)) {
    ...
    if (length(col) == 1) {
      col <- ifelse(is.null(ncol(GGdata[,-1])), col, rep(col, ncol(GGdata[,-1])))
    } else {
