...
#' @param method The membership computation method to be used. One of "fanny",
#'  "cmeans" or "vegclust". Must be present in the ecotonefinder list.
...
NetworkEco <- function (ecotonefinder, threshold = 0.80, plot.type = c("percentage", "corrplot", "heatmap","network"),
                        method = c("cmeans", "vegclust", "fanny"), dist.method = "inner_product", plot = c("species","community"),
                        order.sp = NULL, dist = c("count", "relative", "raw"), no.plot = FALSE, network.group = NULL,
                        method.corr = "number", ...)
{
  returnList <- list()
  returnList[["threshold"]] <- threshold
  if (length(method) > 1 || (method != "cmeans" && method != "vegclust" && method != "fanny")) {
    stop("method should be either fanny, cmeans or vegclust")
  }
  if(is.null(ecotonefinder[[method]])) {
    stop("NetworkEco require the required method of analyses (fanny, vegclust or cmeans) in the imput list")
  }
  if (method == "cmeans") {
    Centers <- ecotonefinder[["cmeans"]][["centers"]]
  }
  if (method == "vegclust") {
    Centers <- ecotonefinder[["vegclust"]][["mobileCenters"]]
  }
  if (method == "fanny") {
    Centers <- ecotonefinder[["fanny"]][["centers"]]
  }
  if (any(dist == c("count","relative")) || any(plot.type == "percentage")) {
    PercentMemb <- list()
    for (i in 1:ncol(Centers)) {
      PercentMemb[[i]] <- t(Centers)[i,]/sum(t(Centers)[i,])
    }
    PercentMemb <- purrr::reduce(PercentMemb, rbind)
    rownames(PercentMemb) <- colnames(ecotonefinder$data)
  ...
  if (dist == "raw") {
    if (plot == "community") {
      Dist <- philentropy::distance(Centers, method = dist.method)
      colnames(Dist) <- rownames(Centers)
      rownames(Dist) <- rownames(Centers)
    }
    if (plot == "species") {
      Dist <- philentropy::distance(t(Centers), method = dist.method)
      colnames(Dist) <- colnames(Centers)
      rownames(Dist) <- colnames(Centers)
    }
...
#'  }
#'
NetworkEcoSeries <- function (ecotonefinder, threshold = 0.80, method = c("cmeans","vegclust", "fanny"), plot.type = c("percentage", "heatmap", "corrplot", "network"),
                              ...
                              dist = c("count", "relative", "raw"), network.group = c("site","cluster"), method.corr = "number", ...)
{
  ...
  if(suppressWarnings(any(lapply(ecotonefinder, function (n) is.null(n$cmeans))) == TRUE &&
                      ...
                      any(lapply(ecotonefinder, function (n) is.null(n$fanny))) == TRUE)) {
    stop("NetworkEcoSeries require cmeans, vegclust or fanny analyses in the input list")
  }
  if (length(method) > 1 || (method != "cmeans" && method != "vegclust" && method != "fanny")) {
    stop("method should be either cmeans, vegclust or fanny")
  }
  if (method == "cmeans") {
    x="cmeans"
    y="centers"
