...
#'   community. If 0 all species curve in the community will have the same b
#'   parameter.
#' @param dev.c The deviation around parameter a for the Gaussian in c
#'   community. If 0 all species curve in the community will have the same c
#'   parameter.
...
#'   to communities. Either a colorspace palette or a vector of the same length
...
#' @param plot Logical. If TRUE, a plot is drawn. Otherwise only the data
#'   are returned.
#'
...
#'                                 pal = c("#008585", "#FBF2C4", "#C7522B"))
#'
#' ### Unequal deviance parameters:
#' SyntheticTrial <- SyntheticData(SpeciesNum = 26, CommunityNum = 2,
#'                                 SpCo=NULL, Length = 500,
#'                                 Parameters=list(a=c(50, 70),
#'                                                 b=c(0, 500),
#'                                                 c=c(0.009, 0.002)),
#'                                 dev.c=c(.0024, .003), dev.a = c(10, 20),
#'                                 dev.b = c(30, 50),
#'                                 pal=c("#008585", "#C7522B"),
#'                                 title = "Asymmetric communities")
#'
#'
                           ...
                           plot = TRUE)
{
  ...
  if (length(dev.a) != 1 && length(dev.a) != CommunityNum) {
    stop("dev.a must be either of length 1 or of the same length as CommunityNum.")
  }
  if (length(dev.b) != 1 && length(dev.b) != CommunityNum) {
    stop("dev.b must be either of length 1 or of the same length as CommunityNum.")
  }
  if (length(dev.c) != 1 && length(dev.c) != CommunityNum) {
    stop("dev.c must be either of length 1 or of the same length as CommunityNum.")
  }
  if (length(dev.a) == 1) {
    dev.a <- rep(dev.a, times = CommunityNum)
  }
  if (length(dev.b) == 1) {
    dev.b <- rep(dev.b, times = CommunityNum)
  }
  if (length(dev.c) == 1) {
    dev.c <- rep(dev.c, times = CommunityNum)
  }
  gaussian <- function(x) a*exp(-(((x-b)^2)/2*(c^2)))
  for (k in 1:CommunityNum) {
    Aseq <- seq(Parameters[[1]][[k]]-dev.a[k], Parameters[[1]][[k]]+dev.a[k], by=1)
    Bseq <- seq(Parameters[[2]][[k]]-dev.b[k], Parameters[[2]][[k]]+dev.b[k], by=1)
    Cseq <- seq(Parameters[[3]][[k]]-dev.c[k], Parameters[[3]][[k]]+dev.c[k], by=0.005)
    for (i in SpList[[k]]) {
      if (dev.a[k] == 0) { a <- Aseq } else { a <- sample(Aseq, 1) }
      if (dev.b[k] == 0) { b <- Bseq } else { b <- sample(Bseq, 1) }
      if (dev.c[k] == 0) { c <- Cseq } else { c <- sample(Cseq, 1) }
...
#' @param ylab A title for the y-axis. See plot.
#' @param title An overall title for the plot. See plot.
#' @param plot Logical. If TRUE, plots are drawn. Otherwise, only the data are
#'   returned.
#'
                                ...
                                plot = TRUE)
{
  ...
  for (j in 1:SeriesNum) {
    ...
    Data[[paste("Time",j,sep = "")]] <- SyntheticData(SpeciesNum = SpeciesNum, CommunityNum = CommunityNum, SpCo = SpCo, Length = Length,
                                                      ...
                                                      xlab = xlab, ylab = ylab, title = title, plot = plot)
    }
  return(Data)
}
